...
class SkillLoaderTest extends TestCase
{
    ...
    public function test_can_load_skill_from_directory(): void
    {
        ...
        $this->assertEquals('pdf-processing', $skill->name);
        $this->assertStringContainsString('Extract text and tables from PDF files', $skill->description);
        $this->assertEquals('MIT', $skill->license);
        $this->assertStringContainsString('PDF Processing Skill', $skill->content);
    ...
    {
        $skill = $this->loader->load(__DIR__.'/Fixtures/data-analysis');

        $this->assertInstanceOf(Skill::class, $skill);
        $this->assertEquals('data-analysis', $skill->name);
        $this->assertStringContainsString('Analyze datasets', $skill->description);
    ...
    public function test_can_load_multiple_skills_from_directory(): void
    {
        ...
        $this->assertContains('pdf-processing', $skillNames);
        $this->assertContains('data-analysis', $skillNames);
    }
    ...
    public function test_skill_to_array_includes_all_fields(): void
    {
        ...
        $this->assertEquals('pdf-processing', $array['name']);
    ...
    public function test_skill_to_xml_generates_proper_format(): void
    {
        ...
        $this->assertStringContainsString('pdf-processing', $xml);
